...
public:
    ...
    S f(const S& x, const C& u) const
    {
        ...
        x_.x() = x.x() - std::sin( newOrientation ) * u.v();
        x_.y() = x.y() + std::cos( newOrientation ) * u.v();
...
protected:
    ...
    void updateJacobians( const S& x, const C& u )
    {
        ...
        this->F( S::X, S::THETA ) = -std::cos( x.theta() + u.dtheta() ) * u.v();
        // partial derivative of x.y() w.r.t. x.y()
        this->F( S::Y, S::Y ) = 1;
        // partial derivative of x.y() w.r.t. x.theta()
        this->F( S::Y, S::THETA ) = -std::sin( x.theta() + u.dtheta() ) * u.v();
