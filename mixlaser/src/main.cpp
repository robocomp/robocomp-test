...
class mixlaser : public RoboComp::Application
{
public:
	mixlaser (QString prfx) { prefix = prfx.toStdString(); }
private:
	void initialize();
	std::string prefix;
	MapPrx mprx;
...
int ::mixlaser::run(int argc, char* argv[])
{
	...
	try
	{
		...
		laser1_proxy = LaserPrx::uncheckedCast( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
	...
	mprx["LaserProxy1"] = (::IceProxy::Ice::Object*)(&laser1_proxy);//Remote server proxy creation example
