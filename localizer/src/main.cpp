 ...
 *
 * Just: "${PATH_TO_BINARY}/Localizer --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class Localizer : public RoboComp::Application
{
public:
	Localizer (QString prfx) { prefix = prfx.toStdString(); }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::Localizer::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		fullposeestimation1_proxy = Ice::uncheckedCast<FullPoseEstimationPrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
